...
function addRow() {
  ...
  xhttp.open(
    "GET",
    "php/add_new_invoice.php?action=add_row&row_id=" +
      cls.value +
      "&row_number=" +
      addRow.counter,
    true
...
function getInvoiceNumber() {
  ...
  xhttp.open(
    "GET",
    "php/add_new_invoice.php?action=current_invoice_number",
    true
  );
  xhttp.send();
}
...
function medicineOptions(text, id) {
  ...
  xhttp.open(
    "GET",
    "php/add_new_invoice.php?action=medicine_list&text=" + text.trim(),
    true
  );
  xhttp.send();
}
...
function fill(name, field_name, column) {
  ...
  xhttp.open(
    "GET",
    "php/add_new_invoice.php?action=fill&name=" + name + "&column=" + column,
    false
  );
  xhttp.send();
}
...
function getTotal(id) {
  ...
  for (var i = 1; i < row_count; i++) {
    qty = Number.parseInt(
      medicine_info[i].children[0].children[4].children[0].value
    );
    mrp = Number.parseFloat(
      medicine_info[i].children[0].children[5].children[0].value
    );
    discount =
      (qty *
        mrp *
        Number.parseFloat(
          medicine_info[i].children[0].children[6].children[0].value
        )) /
      100;
...
function checkAvailableQuantity(value, id) {
  ...
  xhttp.open(
    "GET",
    "php/add_new_invoice.php?action=check_quantity&medicine_name=" +
      medicine_name,
    false
  );
  xhttp.send();
  if (Number.parseInt(xhttp.responseText) == 0) {
    document.getElementById("medicine_name_error_" + id).style.display =
      "block";
    document.getElementById("medicine_name_error_" + id).innerHTML =
      "Out of Stock!";
...
function isCustomer(name, contact_number) {
  ...
  xhttp.open(
    "GET",
    "php/add_new_invoice.php?action=is_customer&name=" +
      name +
      "&contact_number=" +
      contact_number,
    false
...
function isInvoiceExist(invoice_number) {
  ...
  xhttp.open(
    "GET",
    "php/add_new_invoice.php?action=is_invoice&invoice_number=" +
      invoice_number,
    false
  );
  xhttp.send();
...
function isMedicine(name) {
  ...
  xhttp.open(
    "GET",
    "php/add_new_invoice.php?action=is_medicine&name=" + name,
    false
  );
  xhttp.send();
  //alert(xhttp.responseText);
...
function addInvoice() {
  ...
  if (!notNull(customers_name.value, "customer_name_error"))
    ...
    isCustomer(customers_name.value, customers_contact_number.value) == "false"
  ) {
    ...
    for (var i = 1; i < row_count; i++) {
      ...
      var isAvailable = checkAvailableQuantity(
        quantity.value,
        quantity.getAttribute("id").slice(9, 10)
      );
      ...
      if (!notNull(medicine_name.value, medicine_name_error.getAttribute("id")))
        ...
        !checkExpiry(
          expiry_date.value,
          medicine_name_error.getAttribute("id")
        ) ||
        checkExpiry(
          expiry_date.value,
          medicine_name_error.getAttribute("id")
...
function updateStock(name, batch_id, quantity) {
  ...
  xhttp.open(
    "GET",
    "php/add_new_invoice.php?action=update_stock&name=" +
      name +
      "&batch_id=" +
      batch_id +
      "&quantity=" +
...
function addSale(
  ...
  xhttp.open(
    "GET",
    "php/add_new_invoice.php?action=add_sale&customers_name=" +
      customers_name +
      "&customers_contact_number=" +
      customers_contact_number +
      "&invoice_number=" +
